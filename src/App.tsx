...
function MessageRow({ message, agents, t, onRetry, onSkip, onKeep }: { message: LocalMessage; agents: LocalAgent[]; t: ReturnType<typeof translator>; onRetry: () => void; onSkip: () => void; onKeep: () => void }) {
  ...
  return <article className={`message ${message.senderType} ${failed ? 'failed' : ''}`} data-status={message.status}>
    ...
    <div className="message-main"><header><strong>{message.senderType === 'user' ? 'You' : message.senderName}</strong>{message.senderRole && <span>{message.senderRole}</span>}<time>{new Date(message.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></header>
      {message.status === 'waiting' && <p className="state-line"><span className="pulse-dot"/> {message.senderName} {t('waiting')}</p>}
      {message.status === 'thinking' && <p className="state-line"><span className="thinking-dots">•••</span> {message.senderName} {t('thinking')}</p>}
...
function RoomPage({ roomId, t }: { roomId: string; t: ReturnType<typeof translator> }) {
  ...
  const editorValid = enabled.length >= 2 && enabled.length <= 6 && new Set(enabled.map((agent) => agent.normalizedName)).size === enabled.length && enabled.every((agent) => agent.name.trim())
  ...
  const rail = <aside className={`agent-rail ${drawer ? 'open' : ''}`} aria-label={t('agentRail')}>
    <div className="rail-mobile-head"><strong>{t('agentRail')}</strong><button aria-label={t('close')} onClick={() => setDrawer(false)}>×</button></div>
    <div className="rail-room"><span className="eyebrow">{t('agents')}</span>{agents.filter((agent) => agent.enabled).map((agent) => <div className="rail-agent" key={agent.id}><span>{agent.avatar}</span><div><strong>{agent.name}</strong><small>{agent.role}</small></div></div>)}<button className="text-button" onClick={openEditor}>{t('editAgents')} ↗</button></div>
    <div className="rail-controls"><span className="eyebrow">{t('controls')}</span><div className="run-progress"><strong>{run?.turnsCompleted ?? 0}<small> / {run?.turnLimit ?? '—'}</small></strong><span>{t('turns')}</span></div><div className="progress"><i style={{ width: `${run ? (run.turnsCompleted / run.turnLimit) * 100 : 0}%` }}/></div><p>{t('totalTurns')}: {room.totalTurnsCompleted}</p>
      {(room.status === 'paused' || room.status === 'draft') ? <button className="button primary full" onClick={() => void controller.resume()}>{t('resume')} ▶</button> : room.status === 'running' ? <button className="button secondary full" onClick={() => void controller.pause()}>{t('pause')} Ⅱ</button> : null}
      {streaming && <button className="button danger-button full" onClick={() => void controller.stop()}>{t('stop')} ■</button>}
    </div>
  ...
  return <main className="room-shell">{rail}<section className="chat-panel">
    <header className="room-header"><button className="drawer-button" aria-label={t('menu')} onClick={() => setDrawer(true)}>☰</button><div><h1>{room.title}</h1><p>{room.topic}</p></div><span className={`status ${room.status}`}>{t(room.status)}</span></header>
    {!controller.driver && <div className="follower-note">{t('follower')}</div>}{controller.busy && <div className="busy-note">{t('busy')}</div>}{controller.notice && <div className="error-banner" role="alert">{controller.notice}</div>}
  ...
  {editing && <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="agent-modal-title"><header><h2 id="agent-modal-title">{t('editAgents')}</h2><button aria-label={t('close')} onClick={() => setEditing(false)}>×</button></header><div className="modal-scroll">{draftAgents.map((agent, index) => <AgentEditor key={agent.id} agent={agent} index={index} canRemove={draftAgents.length > 2} t={t} onChange={(next) => setDraftAgents((all) => all.map((item) => item.id === agent.id ? next : item))} onRemove={() => setDraftAgents((all) => all.filter((item) => item.id !== agent.id))}/>)}{draftAgents.length < 6 && <button className="button secondary full" onClick={() => setDraftAgents((all) => [...all, blankAgent(roomId)])}>+ {t('addAgent')}</button>}</div><footer><button className="button secondary" onClick={() => setEditing(false)}>{t('cancel')}</button><button className="button primary" disabled={!editorValid} onClick={async () => { await controller.saveAgents(draftAgents); setEditing(false) }}>{t('save')}</button></footer></section></div>}
  </main>
}
